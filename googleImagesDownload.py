...
parser.add_argument('-co', '--cod', help='codigo do arquivo', type=str, required=False)
parser.add_argument('-na', '--name', help='nome do arquivo', type=str, required=False)
...
parser.add_argument('-o', '--output_directory', help='download images in a specific directory', type=str,
                    required=False)
parser.add_argument('-d', '--delay', help='delay in seconds to wait between downloading two images', type=str,
                    required=False)
...
parser.add_argument('-r', '--usage_rights', help='usage rights', type=str, required=False,
                    choices=['labled-for-reuse-with-modifications', 'labled-for-reuse',
                             'labled-for-noncommercial-reuse-with-modification', 'labled-for-nocommercial-reuse'])
...
if args.name:
    nome = [str(item) for item in args.name.split(',')]
if args.cod:
    codigo = [str(item) for item in args.cod.split(',')]
...
def _images_get_all_items(page, maximo):
    items = []
    while True:
        if len(items) >= maximo:
            break
...
def build_url_parameters():
    built_url = "&tbs="
    counter = 0
    params = {'color': [args.color, {'red': 'ic:specific,isc:red', 'orange': 'ic:specific,isc:orange',
              ...
              'type': [args.type, {'face': 'itp:face', 'photo': 'itp:photo', 'clip-art': 'itp:clip-art',
...
# or download multiple images based on keywords
    ...
    while i < len(search_keyword):
        ...
        time.sleep(0.1)
        items = items + (_images_get_all_items(raw_html, limit))
        ...
        while (k < limit):
            try:
                ...
                if ".jpg" in image_name or ".png" in image_name or ".jpeg" in image_name or ".svg" in image_name:
                    ...
                    output_file = open(main_directory + "/" + dir_name + "/" + str(k + 1) + ". " + image_name + ".jpg",
                                       'wb')
                    image_name = image_name + ".jpg"
